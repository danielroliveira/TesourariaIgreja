...
{
	public partial class frmEntradaOrigemProcura : CamadaUI.Modals.frmModFinBorder
	{
		private List<objEntradaOrigem> listOrigem = new List<objEntradaOrigem>();
		private Form _formOrigem;
		public objEntradaOrigem propEscolha { get; set; } //--- PROPRIEDADE DE ESCOLHA

		#region NEW | OPEN FUNCTIONS

		public frmEntradaOrigemProcura(Form formOrigem, int? DefaultID = null)
		{
			InitializeComponent();
		...
		private void ObterDados(object sender, EventArgs e)
		{
			try
			{
				...
				listOrigem = eBLL.GetEntradaOrigemList(true);
				PreencheListagem();
			}
			catch (Exception ex)
			{
		...
		{
			lstItens.DataSource = listOrigem;
			FormataListagem();
		}
		...
		private void FormataListagem()
		{
			...
			clnID.DisplayMember = "IDEntradaOrigem";
			clnID.ValueMember = "IDEntradaOrigem";

			clnItem.DisplayMember = "OrigemDescricao";
			clnItem.ValueMember = "OrigemDescricao";
		...
		{
			objEntradaOrigem item = GetSelectedItem();
		...
		private objEntradaOrigem GetSelectedItem()
		{
			if (lstItens.SelectedItems.Count == 0) return null;

			int IDSelected = (int)lstItens.SelectedItems[0].Value;
			return listOrigem.First(s => s.IDEntradaOrigem == IDSelected);
		}
		...
		private void frmEntradaOrigemProcura_Activated(object sender, EventArgs e)
		{
			if (_formOrigem != null)
			{
		...
		private void frmEntradaOrigemProcura_FormClosed(object sender, FormClosedEventArgs e)
		{
			if (_formOrigem != null)
			{
				Panel pnl = (Panel)_formOrigem.Controls["Panel1"];
		...
		private void ProcurarTexto()
		{
			if (txtProcura.TextLength > 0)
			{
				...
				{
					// declare function
					Func<objEntradaOrigem, bool> FiltroItem = c => c.OrigemDescricao.ToLower().Contains(txtProcura.Text.ToLower());

					// aply filter using function
					lstItens.DataSource = listOrigem.FindAll(c => FiltroItem(c));
				}
				else
				{
					// declare function
					Func<objEntradaOrigem, bool> FiltroID = c => c.CNP.Contains(i.ToString());

					// aply filter using function
					lstItens.DataSource = listOrigem.FindAll(c => FiltroID(c));
				}
			}
			else
			{
