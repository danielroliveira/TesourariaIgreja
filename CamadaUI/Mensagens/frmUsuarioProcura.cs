...
{
	public partial class frmUsuarioProcura : CamadaUI.Modals.frmModFinBorder
	{
		private List<objUsuario> listUser = new List<objUsuario>();
		private Form _formOrigem;
		public objUsuario propEscolha { get; set; } //--- PROPRIEDADE DE ESCOLHA

		#region NEW | OPEN FUNCTIONS

		public frmUsuarioProcura(Form formOrigem, int? DefaultID = null, bool? WithAllUsers = false)
		{
			InitializeComponent();
		...
		private void FormataListagem()
		{
			...
			clnID.DisplayMember = "IDUsuario";
			clnID.ValueMember = "IDUsuario";

			clnUsuario.ValueMember = "UsuarioApelido";
			clnUsuario.DisplayMember = "UsuarioApelido";
		...
		private objUsuario GetSelectedItem()
		{
			...
			if (lstItens.SelectedItems[0].Value != null)
			{
				int IDSelected = (int)lstItens.SelectedItems[0].Value;
				return listUser.First(s => s.IDUsuario == IDSelected);
			}
			else
			{
				return new objUsuario(-1) { UsuarioApelido = "TODOS USUÁRIOS ATIVOS" };
			}
		}
		...
		private void ProcurarTexto()
		{
			if (txtProcura.TextLength > 0)
			{
				...
				{
					// declare function
					Func<objUsuario, bool> FiltroItem = c => c.UsuarioApelido.ToLower().Contains(txtProcura.Text.ToLower());

					// aply filter using function
					lstItens.DataSource = listUser.FindAll(c => FiltroItem(c));
				}
				else
				{
					// declare function
					Func<objUsuario, bool> FiltroID = c => c.IDUsuario == i;

					// aply filter using function
					lstItens.DataSource = listUser.FindAll(c => FiltroID(c));
				}
			}
			else
			{
		...
		private void frmUsuarioProcura_Activated(object sender, EventArgs e)
		{
			if (_formOrigem != null)
			{
		...
		private void frmUsuarioProcura_FormClosed(object sender, FormClosedEventArgs e)
		{
			if (_formOrigem != null)
			{
				Panel pnl = (Panel)_formOrigem.Controls["Panel1"];
