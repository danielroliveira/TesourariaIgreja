...
{
	public partial class frmBancoProcura : CamadaUI.Modals.frmModFinBorder
	{
		private List<objBanco> listBanco = new List<objBanco>();
		private Form _formOrigem;
		public objBanco propEscolha { get; set; } //--- PROPRIEDADE DE ESCOLHA

		#region NEW | OPEN FUNCTIONS

		public frmBancoProcura(Form formOrigem, int? DefaultID = null)
		{
			InitializeComponent();
		...
		private void FormataListagem()
		{
			...
			clnID.DisplayMember = "IDBanco";
			clnID.ValueMember = "IDBanco";

			clnItem.DisplayMember = "BancoNome";
			clnItem.ValueMember = "BancoNome";
		...
		//------------------------------------------------------------------------------------------------------------
		private void frmBancoProcura_KeyDown(object sender, KeyEventArgs e)
		{
		...
		private void frmBancoProcura_Activated(object sender, EventArgs e)
		{
			if (_formOrigem != null)
			{
		...
		private void frmBancoProcura_FormClosed(object sender, FormClosedEventArgs e)
		{
			if (_formOrigem != null)
			{
				Panel pnl = (Panel)_formOrigem.Controls["Panel1"];
		...
		private void ProcurarTexto()
		{
			if (txtProcura.TextLength > 0)
			{
				...
				{
					// declare function
					Func<objBanco, bool> FiltroItem = c => c.BancoNome.ToLower().Contains(txtProcura.Text.ToLower());

					// aply filter using function
					lstItens.DataSource = listBanco.FindAll(c => FiltroItem(c));
				}
				else
				{
					// declare function
					Func<objBanco, bool> FiltroID = c => c.IDBanco == i;

					// aply filter using function
					lstItens.DataSource = listBanco.FindAll(c => FiltroID(c));
				}
			}
			else
			{
